...
void AnimatedSprite::addAnimation(int frames, int x, int y, std::string name, int width, int height, Vector2 offset) {
	...
	//add the rectangle vector into the maps
...
void AnimatedSprite::update(int elapsedTime) {
	...
	if (this->_timeElapsed > this->_timeToUpdate) {
		...
		//if we aren't at the last frameindex, increment _frameIndex by one
		if (this->_frameIndex <this->_animations[this->_currentAnimation].size()-1) {
			this->_frameIndex++;
		}
		//if done we hide it
		//then we reset the frame index and then call animationDone on it
		else {
...
void AnimatedSprite::draw(Graphics &graphics, int x, int y) {
	if (this->_visible) {
		...
		//obtains the rectangle we're drawing this frame
		//first [] pulls the map to get the vector, second [] gets the vector for the rectangle 
